...
addDoc('general', [
  ...
  {sig:'getWindowSize()', desc:'<span class="desctile">获取设计画布尺寸</span>优先获取 .pageContainer 的尺寸，若无则 fallback 到 document.body 的尺寸。',params:[],ret:'{width:number, height:number}',exApi:"var size = webviewBridge.api.getWindowSize();\nconsole.log('画布:', size.width, 'x', size.height);",exCmd:'{}'},
  ...
  {sig:'getPosition(targetId)', desc:'<span class="desctile">获取控件相对于 .pageContainer 的偏移坐标</span>通过 getBoundingClientRect 差值计算，若容器不存在则返回视口坐标。',params:[{n:'targetId',t:'string',d:'目标控件 ID'}],ret:'{x:number, y:number}',exApi:"var pos = webviewBridge.api.getPosition('button_1');\nconsole.log('位置:', pos.x, pos.y);",exCmd:'{\n  "command": "getPosition",\n  "targetId": "button_1"\n}'},
...
addDoc('logOutput', [
  {sig:'logOutput.addLog(targetId, text, color)', desc:'<span class="desctile">向日志框添加一条日志</span>可指定颜色，默认为黑色。',params:[{n:'targetId',t:'string',d:'日志框控件 ID'},{n:'text',t:'string',d:'日志文本'},{n:'color',t:'string',d:'（可选）文本颜色，默认 #000000'}],ret:'boolean — 是否成功',exApi:"// 普通日志\nwebviewBridge.api.logOutput.addLog('logOutput_1', '服务启动成功');\n\n// 彩色日志\nwebviewBridge.api.logOutput.addLog('logOutput_1', '错误：连接失败', '#ff0000');\nwebviewBridge.api.logOutput.addLog('logOutput_1', '警告：磁盘空间不足', '#ff8800');",exCmd:'{\n  "command": "logOutput.addLog",\n  "targetId": "logOutput_1",\n  "text": "错误：连接失败",\n  "color": "#ff0000"\n}'},
  {sig:'logOutput.clearLog(targetId)', desc:'<span class="desctile">清空日志框所有日志</span>',params:[{n:'targetId',t:'string',d:'日志框控件 ID'}],ret:'boolean — 是否成功',exApi:"webviewBridge.api.logOutput.clearLog('logOutput_1');",exCmd:'{\n  "command": "logOutput.clearLog",\n  "targetId": "logOutput_1"\n}'},
  {sig:'logOutput.getLogCount(targetId)', desc:'<span class="desctile">获取日志框中的日志条数</span>',params:[{n:'targetId',t:'string',d:'日志框控件 ID'}],ret:'number — 日志条数',exApi:"var count = webviewBridge.api.logOutput.getLogCount('logOutput_1');\nconsole.log('日志数:', count);",exCmd:'{\n  "command": "logOutput.getLogCount",\n  "targetId": "logOutput_1"\n}'}
]);
...
addDoc('listBox', [
  ...
  {sig:'listBox.showCheckbox(targetId, show)', desc:'<span class="desctile">显示/隐藏列表项前的复选框列</span>设置 data-show-checkbox 属性。DOM 中无复选框时自动为每个 .listBox_item 动态创建；已存在则控制 display 显隐。后续 addItem 会根据该属性自动生成复选框。',params:[{n:'targetId',t:'string',d:'列表框控件 ID'},{n:'show',t:'boolean',d:'true=显示, false=隐藏'}],ret:'boolean — 是否成功',exApi:"// 显示复选框（无则自动创建）\nwebviewBridge.api.listBox.showCheckbox('listBox_1', true);\n\n// 隐藏复选框\nwebviewBridge.api.listBox.showCheckbox('listBox_1', false);",exCmd:'{\n  "command": "listBox.showCheckbox",\n  "targetId": "listBox_1",\n  "show": true\n}'},
...
addDoc('treeView', [
  ...
  {sig:'treeView.deselectNode(targetId, nodeId)', desc:'<span class="desctile">取消指定树节点的选中高亮</span>移除该节点的 .treeView_node_content .selected 类名。',params:[{n:'targetId',t:'string',d:'树形框控件 ID'},{n:'nodeId',t:'string',d:'节点 ID'}],ret:'boolean — 是否成功',exApi:"webviewBridge.api.treeView.deselectNode('treeView_1', 'node_2');",exCmd:'{\n  "command": "treeView.deselectNode",\n  "targetId": "treeView_1",\n  "nodeId": "node_2"\n}'},
  {sig:'treeView.clearSelection(targetId)', desc:'<span class="desctile">清除树形框当前的选中高亮</span>移除 .treeView_node_content 上的 .selected 类名。',params:[{n:'targetId',t:'string',d:'树形框控件 ID'}],ret:'boolean — 是否成功',exApi:"webviewBridge.api.treeView.clearSelection('treeView_1');",exCmd:'{\n  "command": "treeView.clearSelection",\n  "targetId": "treeView_1"\n}'},
  ...
  {sig:'treeView.enableAllEdit(targetId)', desc:'<span class="desctile">启用整个树形框的编辑功能</span>设置容器 data-editable="true"，同时给所有 .treeView_label 设置 contenteditable="true"。',params:[{n:'targetId',t:'string',d:'树形框控件 ID'}],ret:'boolean — 是否成功',exApi:"webviewBridge.api.treeView.enableAllEdit('treeView_1');",exCmd:'{\n  "command": "treeView.enableAllEdit",\n  "targetId": "treeView_1"\n}'},
  {sig:'treeView.disableAllEdit(targetId)', desc:'<span class="desctile">禁用整个树形框的编辑功能</span>设置容器 data-editable="false"，同时移除所有 .treeView_label 的 contenteditable 属性。',params:[{n:'targetId',t:'string',d:'树形框控件 ID'}],ret:'boolean — 是否成功',exApi:"webviewBridge.api.treeView.disableAllEdit('treeView_1');",exCmd:'{\n  "command": "treeView.disableAllEdit",\n  "targetId": "treeView_1"\n}'},
  ...
  {sig:'treeView.showCheckbox(targetId, show)', desc:'<span class="desctile">显示/隐藏树形框节点的复选框</span>设置 data-show-checkbox 属性。DOM 中无复选框时自动为每个 .treeView_node_content 动态创建；已存在则控制显隐。后续 addNode 会根据该属性自动生成复选框。',params:[{n:'targetId',t:'string',d:'树形框控件 ID'},{n:'show',t:'boolean',d:'true=显示, false=隐藏'}],ret:'boolean — 是否成功',exApi:"// 显示复选框（无则自动创建）\nwebviewBridge.api.treeView.showCheckbox('treeView_1', true);\n\n// 隐藏复选框\nwebviewBridge.api.treeView.showCheckbox('treeView_1', false);",exCmd:'{\n  "command": "treeView.showCheckbox",\n  "targetId": "treeView_1",\n  "show": true\n}'},
...
addDoc('dataGrid', [
  ...
  {sig:'dataGrid.selectRow(targetId, rowIndex)', desc:'<span class="desctile">视觉高亮指定行（添加 .dataGrid_row-focused 类，不改变复选框状态）</span>先清除所有行的高亮再设置目标行。',params:[{n:'targetId',t:'string',d:'表格控件 ID'},{n:'rowIndex',t:'number',d:'行索引'}],ret:'boolean — 是否成功',exApi:"webviewBridge.api.dataGrid.selectRow('dataGrid_1', 2);",exCmd:'{\n  "command": "dataGrid.selectRow",\n  "targetId": "dataGrid_1",\n  "rowIndex": 2\n}'},
  {sig:'dataGrid.deselectRow(targetId, rowIndex)', desc:'<span class="desctile">取消指定行的视觉高亮（移除 .dataGrid_row-focused 类）</span>',params:[{n:'targetId',t:'string',d:'表格控件 ID'},{n:'rowIndex',t:'number',d:'行索引'}],ret:'boolean — 是否成功',exApi:"webviewBridge.api.dataGrid.deselectRow('dataGrid_1', 2);",exCmd:'{\n  "command": "dataGrid.deselectRow",\n  "targetId": "dataGrid_1",\n  "rowIndex": 2\n}'},
...
addDoc('cardBox', [
  ...
  {sig:'cardBox.showCollapseButton(targetId, show)', desc:'<span class="desctile">显示/隐藏卡片框的折叠箭头按钮</span>通过控制 .cardBox_collapse_btn 的 display 属性实现。若按钮不存在返回 false。',params:[{n:'targetId',t:'string',d:'卡片框控件 ID'},{n:'show',t:'boolean',d:'true=显示, false=隐藏'}],ret:'boolean — 是否成功',exApi:"// 隐藏折叠箭头\nwebviewBridge.api.cardBox.showCollapseButton('cardBox_1', false);\n\n// 显示折叠箭头\nwebviewBridge.api.cardBox.showCollapseButton('cardBox_1', true);",exCmd:'{\n  "command": "cardBox.showCollapseButton",\n  "targetId": "cardBox_1",\n  "show": true\n}'},
