...
(function() {
  ...
  // ================================================================
  // Layer 3: 原生 HTML 标签 → ctrlType 映射表（v2 新增）
  // ================================================================
  var NATIVE_TAG_MAP = {
    'BUTTON': 'button',
    'A': 'hyperLink',
  ...
  // source 标记常量（v2 新增）
  var SOURCE_EXPLICIT = 'explicit';    // data-ctrl-type 显式声明
  var SOURCE_NATIVE = 'native';         // 原生 HTML 标签自动推断
  var SOURCE_CONTAINER = 'container';   // 复杂控件容器内子元素
  var SOURCE_CSS = 'css';               // CSS class 检测
  ...
  function detectCtrlType(el) {
    ...
    while (current) {
      if (current.hasAttribute && current.hasAttribute('data-ctrl-type')) {
        return { ctrlType: current.getAttribute('data-ctrl-type'), source: SOURCE_EXPLICIT };
      }
      if (current.classList) {
        if (current.classList.contains('treeView_toggle')) return { ctrlType: 'treeview_node_toggle', source: SOURCE_CSS };
        if (current.classList.contains('treeView_label')) return { ctrlType: 'treeview_node_text', source: SOURCE_CSS };
        if (current.classList.contains('treeView_node_content')) {
          var lbl = current.querySelector('.treeView_label');
          if (lbl && lbl === el) return { ctrlType: 'treeview_node_text', source: SOURCE_CSS };
          var tgl = current.querySelector('.treeView_toggle');
          if (tgl && tgl === el) return { ctrlType: 'treeview_node_toggle', source: SOURCE_CSS };
        }
        if (current.classList.contains('listBox_scroll')) return { ctrlType: 'listBox', source: SOURCE_CSS };
        if (current.classList.contains('listBox_item')) return { ctrlType: 'listBox_item', source: SOURCE_CSS };
        if (current.classList.contains('listBox_item_text')) return { ctrlType: 'listBox_item', source: SOURCE_CSS };
        if (current.classList.contains('listBox_item-checkbox')) return { ctrlType: 'listbox_item_checkbox', source: SOURCE_CSS };
        if (current.classList.contains('logOutput_container')) return { ctrlType: 'logOutput', source: SOURCE_CSS };
        if (current.classList.contains('logOutput_line')) return { ctrlType: 'logOutput_item', source: SOURCE_CSS };
        if (current.classList.contains('dataGrid_container')) return { ctrlType: 'dataGrid', source: SOURCE_CSS };
        if (current.classList.contains('dataGrid_body')) return { ctrlType: 'dataGrid', source: SOURCE_CSS };
        if (current.classList.contains('dataGrid_header')) return { ctrlType: 'dataGrid', source: SOURCE_CSS };
        if (current.classList.contains('dataGrid_cell')) return { ctrlType: 'dataGrid_cell', source: SOURCE_CSS };
        if (current.classList.contains('dataGrid_row')) return { ctrlType: 'dataGrid', source: SOURCE_CSS };
        if (current.classList.contains('treeView_children')) return { ctrlType: 'treeView', source: SOURCE_CSS };
        if (current.classList.contains('treeView_node')) return { ctrlType: 'treeView', source: SOURCE_CSS };
        if (current.classList.contains('pageContainer')) return { ctrlType: 'pageContainer', source: SOURCE_CSS };
        if (current.classList.contains('progressBar_container')) return { ctrlType: 'progressBar', source: SOURCE_CSS };
      }
      current = current.parentElement;
    }
    // Layer 3: 原生 HTML 标签推断（兜底）（v2 新增）
    var inferred = inferCtrlTypeFromTag(el);
    if (inferred) return { ctrlType: inferred, source: SOURCE_NATIVE };
    return { ctrlType: '', source: '' };
  }
  ...
  function send(action, ctrlType, targetId, data) {
    ...
    // v2: 自动推断 source
    var source = SOURCE_EXPLICIT;
    if (targetEl) {
      if (!targetEl.hasAttribute('data-ctrl-type') && !targetEl.closest('[data-ctrl-type]')) {
        source = SOURCE_NATIVE;
      }
    }
    var dataObj = data || {};
    // v2: source 标记注入 data 子对象（不改变顶层消息结构，兼容 v1）
    dataObj._source = source;
    var msg = {
  ...
  function handleClick(e) {
    var dtResult = detectCtrlType(e.target);
    var ctrlType = dtResult.ctrlType;
    if (!ctrlType) return;
  ...
  function dispatchCommand(cmd) {
    ...
    // 1. 首先检查 defaultCommands（最高优先级，保留所有旧命令）
    //    如 setValue, show, hide, maximize, messageBox.getConfig 等
    ...
    // 2. v2: 统一路由 — 控件ID.方法名
    //    格式：{ command: "listBox_1.addItem", value: "xxx", needResult: true }
    var dotIdx = name.indexOf('.');
    if (dotIdx > 0) {
      var widgetId = name.substring(0, dotIdx);
      var method = name.substring(dotIdx + 1);
      var widget = getWidget(widgetId);
      
      if (widget && typeof widget[method] === 'function') {
        try {
          var result = widget[method](cmd);
          if (cmd.needResult) {
            send('cmdResult', '', widgetId, {
              command: name,
              result: deepProcess(result, IconManager.toText)
            });
          }
        } catch(e) {
          warn('[webviewBridge] 控件方法执行异常:', name, e.message);
        }
        return;
      }
      
      // 回退到通用API：api[method](widgetId, cmd)
      if (window.webviewBridge && window.webviewBridge.api) {
        var apiMethod = window.webviewBridge.api[method];
        if (typeof apiMethod === 'function') {
          try {
            var apiResult = apiMethod(widgetId, cmd);
            if (cmd.needResult) {
              send('cmdResult', '', widgetId, {
                command: name,
                result: deepProcess(apiResult, IconManager.toText)
              });
            }
          } catch(e) {
            warn('[webviewBridge] API方法执行异常:', name, e.message);
          }
          return;
        }
      }
    ...
    // 3. 通用点号命令分发（旧 v1 路径兼容）
    //    如 tabContainer.selectTab、dataGrid.addRow 等
  ...
  // ================================================================
  // 容器渲染引擎 — 扫描 data-ctrl 并自动渲染（v2 新增）
  // ================================================================

  function renderContainers() {
    var containers = document.querySelectorAll('[data-ctrl]');
    for (var i = 0; i < containers.length; i++) {
      var el = containers[i];
      var ctrl = el.getAttribute('data-ctrl');
      if (!ctrl) continue;
      
      switch (ctrl) {
        case 'listBox': renderListBox(el); break;
        case 'dataGrid': renderDataGrid(el); break;
        case 'treeView': renderTreeView(el); break;
  ...
  function renderListBox(el) {
    ...
    var html = '<div class="listBox_scroll">';
    for (var i = 0; i < items.length; i++) {
      html += '<div class="listBox_item" data-ctrl-type="listBox_item" data-item-index="' + i + '">';
      if (showCheckbox) {
        html += '<input type="checkbox" class="listBox_item-checkbox" data-ctrl-type="listbox_item_checkbox" />';
      }
      html += '<span class="listBox_item_text">' + escapeHtml(items[i]) + '</span>';
      html += '</div>';
    }
    html += '</div>';
    el.innerHTML = html;
    el.setAttribute('data-ctrl-type', 'listBox');
    el.setAttribute('data-editable', editable ? 'true' : 'false');
    el.setAttribute('data-always-show-selection', 'true');
    
    // 注册控件API
    registerWidget(el.id, {
      el: el,
      getValue: function() {
        var sel = el.querySelector('.item-selected');
        return sel ? sel.querySelector('.listBox_item_text').textContent : '';
      },
      setValue: function(val) { /* 选中匹配项 */ },
      getData: function() { return items; },
      setItems: function(newItems) {
        el.setAttribute('data-items', JSON.stringify(newItems));
        renderListBox(el);
      },
  ...
  // 渲染表格（v2 新增）
  function renderDataGrid(el) {
    var columns = [];
    var rows = [];
    try { columns = JSON.parse(el.getAttribute('data-columns') || '[]'); } catch(e) {}
    try { rows = JSON.parse(el.getAttribute('data-rows') || '[]'); } catch(e) {}
    var showCheckbox = el.getAttribute('data-show-checkbox') === 'true';
    var editable = el.getAttribute('data-editable') === 'true';
    
    var html = '<div class="dataGrid_container">';
    // 表头
    html += '<div class="dataGrid_header">';
    if (showCheckbox) {
      html += '<div class="dataGrid_header_cell dataGrid_checkbox" style="width:36px;min-width:36px;flex-shrink:0"><input type="checkbox" class="dataGrid_select_all"></div>';
    }
    for (var c = 0; c < columns.length; c++) {
      html += '<div class="dataGrid_header_cell" data-ctrl-type="dataGrid_cell" data-col-key="col' + c + '" style="min-width:80px;flex:1">' + escapeHtml(columns[c]) + '</div>';
    }
    html += '</div>';
    
    // 表体
    html += '<div class="dataGrid_body">';
    for (var r = 0; r < rows.length; r++) {
      var rowId = el.id + '_row_' + (r + 1);
      html += '<div class="dataGrid_row" data-row-index="' + r + '" data-row-id="' + rowId + '">';
      if (showCheckbox) {
        html += '<div class="dataGrid_cell dataGrid_checkbox" style="width:36px;min-width:36px;flex-shrink:0"><input type="checkbox" class="dataGrid_row_check" data-ctrl-type="dataGrid_row_checkbox"></div>';
      }
      for (var cc = 0; cc < columns.length; cc++) {
        var cellVal = (rows[r] && rows[r][cc] !== undefined) ? rows[r][cc] : '';
        html += '<div class="dataGrid_cell" data-ctrl-type="dataGrid_cell" data-col-key="col' + cc + '" style="min-width:80px;flex:1" title="' + escapeHtml(String(cellVal)) + '">' + escapeHtml(String(cellVal)) + '</div>';
      }
      html += '</div>';
    }
    html += '</div></div>';
    el.innerHTML = html;
    el.setAttribute('data-ctrl-type', 'dataGrid');
    el.className = (el.className || '') + ' dataGrid_container';
    el.setAttribute('data-editable', editable ? 'true' : 'false');
    
    registerWidget(el.id, {
      el: el,
      getValue: function() {
        var sel = el.querySelector('.dataGrid_row-focused');
        return sel ? sel.getAttribute('data-row-id') : '';
      },
      getData: function() { return { columns: columns, rows: rows }; },
      setRows: function(newRows) {
        el.setAttribute('data-rows', JSON.stringify(newRows));
        renderDataGrid(el);
      },
      addRow: function(row) {
        rows.push(row || []);
  ...
  // 渲染树形框（v2 新增）
  function renderTreeView(el) {
    var tree = [];
    try { tree = JSON.parse(el.getAttribute('data-tree') || '[]'); } catch(e) {}
    var showIcon = el.getAttribute('data-show-icon') !== 'false';
    var editable = el.getAttribute('data-editable') === 'true';
    
    var nodeIdCounter = 0;
    function buildTreeNodes(nodes, level) {
      var html = '';
      for (var i = 0; i < nodes.length; i++) {
        var node = nodes[i];
        var nodeId = 'node_' + (++nodeIdCounter);
        var hasChildren = node.children && node.children.length > 0;
        var toggleClass = hasChildren ? 'expanded' : 'empty';
        
        html += '<div class="treeView_node" data-node-id="' + nodeId + '" data-level="' + level + '">';
        html += '<div class="treeView_node_content">';
        html += '<span class="tree-indent" style="width:' + (level * 20) + 'px;display:inline-block;flex-shrink:0"></span>';
        html += '<span class="treeView_toggle ' + toggleClass + '" data-ctrl-type="treeview_node_toggle">▶</span>';
        if (showIcon) {
          html += '<span class="treeView_icon">' + (hasChildren ? '📁' : '📄') + '</span>';
        }
        html += '<span class="treeView_label" data-ctrl-type="treeview_node_text">' + escapeHtml(node.text) + '</span>';
        html += '</div>';
        if (hasChildren) {
          html += '<div class="treeView_children" style="display:block">';
          html += buildTreeNodes(node.children, level + 1);
          html += '</div>';
        }
        html += '</div>';
      }
    ...
    el.innerHTML = buildTreeNodes(tree, 0);
    el.setAttribute('data-ctrl-type', 'treeView');
    el.setAttribute('data-type', 'treeView');
    el.setAttribute('data-editable', editable ? 'true' : 'false');
    ...
    registerWidget(el.id, {
      ...
      getValue: function() {
        ...
        if (sel) {
          var label = sel.querySelector('.treeView_label');
          return label ? label.textContent : '';
        }
        return '';
      },
      getData: function() { return tree; },
      setTree: function(newTree) {
        el.setAttribute('data-tree', JSON.stringify(newTree));
        renderTreeView(el);
      }
    });
  }
  ...
  function renderTabs(el) {
    var tabs = [];
    try { tabs = JSON.parse(el.getAttribute('data-tabs') || '[]'); } catch(e) {}
    
    var html = '<div class="tabsContainer" data-ctrl-type="tabsContainer">';
    // 标签头
    html += '<div class="tabsContainer_headerBar">';
    for (var i = 0; i < tabs.length; i++) {
      html += '<button class="tabsContainer_headerBar_btn' + (i === 0 ? ' active' : '') + '" data-ctrl-type="tabsContainer_headerBar_btn" data-tab-name="tab' + i + '">' + escapeHtml(tabs[i].title) + '</button>';
    }
    html += '</div>';
    // 标签内容
    html += '<div class="tabsContainer_contentWrapper">';
    for (var j = 0; j < tabs.length; j++) {
      html += '<div class="tabsContainer_contentWrapper_panel' + (j === 0 ? ' active' : '') + '" data-tab-name="tab' + j + '" data-parent="' + el.id + '" data-tab="' + j + '">';
      html += tabs[j].content || '';
      html += '</div>';
    }
    html += '</div></div>';
    el.innerHTML = html;
    el.setAttribute('data-ctrl-type', 'tabsContainer');
    
    registerWidget(el.id, {
      el: el,
      getValue: function() {
        var active = el.querySelector('.tabsContainer_headerBar_btn.active');
        return active ? active.getAttribute('data-tab-name') : '';
      },
      selectTab: function(index) {
        var btns = el.querySelectorAll('.tabsContainer_headerBar_btn');
        var panels = el.querySelectorAll('.tabsContainer_contentWrapper_panel');
